...
class CorrelationBacktester:
    ...
    def evaluate_regime_accuracy(self) -> Dict[str, float]:
        ...
        true_positive = (
            (self.results["regime"] == "Stress") & (self.results["actual_stress"])
        ).sum()
    ...
    def plot_regime_timeline(self, save_path: str = None):
        ...
        axes[0].plot(
            self.results.index,
            self.results["vol_ratio"],
            color="black",
            linewidth=0.8,
            label="Vol Ratio",
        )
        axes[0].axhline(
            y=1.5, color="red", linestyle="--", label="Stress Threshold (1.5x)"
        )
        axes[0].fill_between(
            self.results.index,
            0,
        ...
        axes[1].plot(
            self.results.index,
            self.results["market_credit_corr"],
            color="blue",
            linewidth=0.8,
        )
        axes[1].axhline(y=0.30, color="green", linestyle="--", label="Normal (0.30)")
        axes[1].axhline(y=0.65, color="red", linestyle="--", label="Stress (0.65)")
        axes[1].fill_between(
            self.results.index, 0, 1, where=stress_mask, alpha=0.3, color="red"
        )
    ...
    def generate_report(self) -> str:
        ...
        report.append("-" * 70)
        report.append(
            f"Precision:  {metrics['precision']:.2%}  (of detected stress, % actually stress)"
        )
        report.append(
            f"Recall:     {metrics['recall']:.2%}  (of actual stress, % detected)"
        )
